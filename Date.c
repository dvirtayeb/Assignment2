...
int userAddDate(Date *date, char *str) { // Check if the Date Format right and add the date
	int flag = 1;
	...
	while (temp != NULL && flag == 1) {
		flag = checkDigit(temp, &counterValidDate);
		if (!flag)
			break;
...
int checkDigit(const char *temp, const int *counterValidDate) {
...
void insertIntegerValues(Date *date, const char *temp, int *counterValidDate) {
